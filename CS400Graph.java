...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path source) {
      ...
      this.dataSequence = new LinkedList<>(source.dataSequence); // creating a separate linked list
    ...
    public void extend(Edge edge) {
      dataSequence.add(edge.target.data); // adding the edge's target vertex to the sequence 
      end = edge.target; // since edge is the last addition, it should be the end
      distance = distance + edge.weight; // update the weight to include the newly added edge's
                                         // weight
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
    if (!(containsVertex(start)) || !(containsVertex(end))) { // make sure that the passed vertices
                                                              // exist
      throw new NoSuchElementException("Vertices do not exist");
    }

    PriorityQueue<Path> pq = new PriorityQueue<Path>();
    Hashtable<T, Path> visited = new Hashtable<T, Path>();

    Vertex startVertex = vertices.get(start);
    Path startPath = new Path(startVertex);

    visited.put(startPath.end.data, startPath); // since we start at the first path, it is visited
    pq.add(startPath); // first path (distance 0) added to the priority queue
    Path currentPath = startPath; // currently you are at the starting path


    while (!pq.isEmpty() && !currentPath.end.data.equals(end)) { // pq consists of paths and you are
                                                                 // not at the end yet
      Path min = pq.poll(); // poll returns the min value path
      if (min.end.edgesLeaving.isEmpty() && pq.isEmpty()) { // if you're at a "dead end" and there
                                                            // is nothing else in the pq, the
                                                            // current path you're on is just the
                                                            // minimum path
        currentPath = min;
      } else {

        for (Edge e : min.end.edgesLeaving) { // iterate through each edge leaving the vertex
          if (!visited.contains(e.target)) { // if the target vertex has not been visited yet ...
            Path nextPath = new Path(min); // create a new copy path for each path you encounter
            nextPath.extend(e); // extend the path by the edge, and then add to the pq
            pq.add(nextPath);
          }
        }
        currentPath = pq.peek(); // without removing the shortest path in pq, save it in current
                                 // path
        visited.put(currentPath.end.data, currentPath);
      }
    }

    if (!(currentPath.end.data.equals(end))) { // if you are still not at the desired end, that means there
                                       // is no path
      throw new NoSuchElementException("Does not exist.");
    }

    return currentPath; // currentPath should be storing the shortest path from peeking in line 365
  }
